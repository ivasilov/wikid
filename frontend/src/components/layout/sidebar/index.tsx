import { faBookmark, faBookOpen, faHouse, faPlus } from '@fortawesome/free-solid-svg-icons';
import Link from 'next/link';
import { Icon } from '../../icon';
import { PageCategories } from './page-categories';

export const Sidebar = () => {
  return (
    <div className="col-span-2 overflow-y-auto bg-[#f2f4f7] min-h-screen">
      <div className="flex">
        <div className="h-10 w-10 bg-[#343b4b] text-base text-white px-3 py-2.5">W</div>
        <div className="flex justify-end flex-grow px-3 py-2.5">
          <Link href="/bookmarks/new">
            <Icon name={faPlus} />
          </Link>
        </div>
      </div>

      <div className="pt-8 pl-5 list-none text-sm font-normal">
        <div className="pb-4">
          <Icon name={faHouse} className="pr-2 text-slate-400" />
          <Link href="/bookmarks">
            <a>All bookmarks</a>
          </Link>
        </div>
        <div className="pb-4">
          <Icon name={faBookmark} className="pr-2 text-slate-400" />
          Untagged bookmarks
        </div>
        <div className="pb-4">
          <Icon name={faBookOpen} className="pr-2 text-slate-400" />
          <Link href="/bookmarks/unread">Reading list</Link>
        </div>
        <PageCategories />
      </div>
    </div>
  );
};
